			...
			<div id="caja2">
				<article>
					<?php
						...
						$querybuscarD = $mysqli->query("SELECT * FROM datospersonales 
					    where id=$id") or die ( "<br>No se puede ejecutar query 
					    para buscar datos personales ". $mysqli->error);

					    $querybuscarU = $mysqli->query("SELECT * FROM usuario 
					    where id=$id") or die ( "<br>No se puede ejecutar query 
					    para buscar  usuario ". $mysqli->error);
					...
					<fieldset>
						...
						<?php
						// Campo Oculto ID
						echo "<input disabled type='hidden' name='hiddenid' value='$id'>";
						
						echo "<label>Cedula :
								<input disabled name='txtcedula' type='text' id='txtnombre' size='12' maxlength='8' disabled placeholder='ingrese cedula' value='$cedula'>
						</label>";
						// Campo Oculto Cedula
						echo "<input disabled type='hidden' name='hiddenced' value='$cedula'>";
						...
						<?php
						echo "<label>Nombre :
								<input disabled name='txtnombre' type='text' id='txtnombre' size='30' maxlength='20'  placeholder='ingrese nombre' value='$nombre'>
						</label>";
						?>
						<?php
						...
						<?php
						echo "<label>Apellido :
								<input disabled name='txtapellido' type='text' id='txtapellido' size='30' maxlength='20'  placeholder='ingrese apellido' value='$apellido'>
						</label>";
						?>
						<?php
						// el isset determina si una variable está definida, es decir posee algún valor, en cuyo caso nos devolvera el valor booleano.
						...
						// Validacion de Radiobox
							...
							echo "<input disabled name='RadOpcion'  type='radio' id='RadOpcion' value='F' ";
								if($RadOpcion === 'F')
								{
								  echo "checked ";
								}
							...
							echo "<input disabled name='RadOpcion'  type='radio' id='RadOpcion' value='M' ";
								if($RadOpcion === 'M')
								{
								  echo "checked ";
								}
						...
						<?php
						 ...
						 echo	"<label>Hobies : </label> <br />  ";
							...
							while (($fila=mysqli_fetch_array($querybuscarH)))
							{
								...
								echo "$nombrehobie : <input disabled name='CheckBox[]'  type='checkbox' value='$idhobie' ";
						...
						// Validad Seleccion en Php
							echo	"<label><br />Estado Civil : </label> <br />
								<select disabled name='seleccion_simple' >
						...
						<?php
						echo "<label>fechanac :
								<input disabled name='txtfechanac' type='date' id='txtfechanac' size='12' maxlength='8'  placeholder='ingrese fecha nac.' value='$fechanac'>
						</label>";
						...
						<?php
						echo "<label>correo :
								<input disabled name='txtcorreo' type='email' id='txtcorreo' size='80' maxlength='80'  placeholder='ingrese correo electrónico' value='$correo'>
						</label>";
						...
						echo "<label>Nombre de Usuario :
								<input disabled name='txtusuario' type='text' id='txtusuario' size='30' maxlength='20'  placeholder='ingrese el usuario' value='$usuario'>
						</label>";
						?>
						<?php
						// el isset determina si una variable está definida, es decir posee algún valor, en cuyo caso nos devolvera el valor booleano.
						...
						<?php
						echo "<label>Clave :
								<input disabled name='txtclave1' type='text' id='txtclave1' size='30' maxlength='20'  placeholder='ingrese la clave' value='$clave'>
						</label>";
						?>
						<?php
						// el isset determina si una variable está definida, es decir posee algún valor, en cuyo caso nos devolvera el valor booleano.
						...
						echo "<label>Repetir Clave :
								<input disabled name='txtclave2' type='text' id='txtclave2' size='30' maxlength='20'  placeholder='ingrese la clave' ";
								if (isset($_POST['BtnEnviar'])) 
								{
									echo "value='$clave2'>";
								}else
						...
						if ( isset ($_POST['BtnEnviar']) )
						{
							...
							{
								echo "<br /><br /><label><input name='BtnEnviar' type='submit' id='BtnEnviar' value='Eliminar' /></label>";
							}
							else
							{
								$queryEliminar = $mysqli->query("DELETE from datospersonales where id='$id'")
								or die ("<br>No se puede ejecutar query para eliminar datos personales". $mysqli->error);

								$queryEliminar2 = $mysqli->query("DELETE from usuario where id='$id'")
								or die ("<br>No se puede ejecutar query para eliminar datos usuario". $mysqli->error);

								$queryEliminar3 = $mysqli->query("DELETE from hobiesdp where id='$id'")
								or die ("<br>No se puede ejecutar query para eliminar datos hobie". $mysqli->error);
										
										
										if ($queryEliminar && $queryEliminar2 )
										{
