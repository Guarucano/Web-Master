					...
					<fieldset>
						...
						?>	
						 ...
						 // Buscar en la Base de Datos
							$querybuscarH = $mysqli->query("SELECT * FROM hobies ") 
							or die ( "<br>No se puede ejecutar query para buscar
							Hobies ". $mysqli->error);
							while (($fila=mysqli_fetch_array($querybuscarH)))
							{
								$idhobie= $fila['idhobie'];
								$nombrehobie= $fila['nombrehobie'];
								echo "$nombrehobie : <input name='CheckBox[]' type='checkbox'
								value='$idhobie' ";
						...
						if ( isset ($_POST['BtnEnviar']) )
						{
								...
								else
								{
									...
									if (mysqli_num_rows($querybuscar) > 0 )
									{
										...
										where usuario='$usuario' ") or die ( "<br>No se puede ejecutar query
									...
									else
									{
										...
										{
											// Insertar en la Base de Datos
											$queryInsertar3 = $mysqli->query("INSERT INTO 
											hobiesdp( idhobiedp, id, idhobie ) 
											values ( null, '$id', '$idh' )") 
											or die ( "<br>No se puede ejecutar query 
											para insertar datos hobies". $mysqli->error);
										}
