				...
				<article>
					...
					?>		
						...
						$querybuscarU = $mysqli->query("SELECT * FROM datospersonales 
								...
								para buscar datos P ". $mysqli->error);
						}
						if ( isset ($_POST['BtnEnviar']) && $seleccion_buscar == 'todo' )
						{
							...
							if (mysqli_num_rows($querybuscarU) > 0 )
							{
								echo "<table>";
									...
									or die ( "<br>No se puede ejecutar query para 
									buscar hobies ". $mysqli->error);
									...
									while (($fila=mysqli_fetch_array($querybuscarU)))
									{
										...
										where id='$id' order by idhobiedp") 
										or die ( "<br>No se puede ejecutar query para buscar
										datos hobies ". $mysqli->error);
